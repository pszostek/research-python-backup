...
class JrsMlHierarchicalWrapper(object):
    '''
    ...
    def __init__(self, knn_callable, distances, train_labels_input, label_mappings, continue_deepening, 
                 is_leaf_node, return_label_mapping = lambda x: int(x)):
        '''
        distances - list of lists of distances
            for example distance between x and y: distances[x][y].
        '''
        self.return_label_mapping = return_label_mapping
    ...
    def classify(self, sample_distances):
        ...
        return map(self.return_label_mapping, classification_result)
...
if __name__ == "__main__":
    ...
    knn_callable = lambda records, labels, list_of_all_labels: FractionKnnJrsTrained(distances, records, labels, k, list_of_all_labels)
    mlknn_jrs = JrsMlHierarchicalWrapper(knn_callable, distances, labels, label_mappings, continue_deepening, is_leaf_node, lambda x: x)
    ...
    #min_weight = 0.3
    k_list = [3]#, 5, 7]
    knn_callable = lambda records, labels, list_of_all_labels: FractionKnnJrsTrainedSemiEnsembledStrongestWins(distances, records, labels, k_list, list_of_all_labels)
    mlknn_jrs = JrsMlHierarchicalWrapper(knn_callable, distances, labels, label_mappings, continue_deepening, is_leaf_node, lambda x: x)
    #print mlknn_jrs.fmeasure_per_class
    print mlknn_jrs.classify([1, 3, 3, 1, 2, 4, 6, 9, 11])
    print mlknn_jrs.classify([111, 113, 113, 111, 2, 4, 1, 1, 11])