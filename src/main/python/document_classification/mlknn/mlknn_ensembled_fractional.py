...
class MlKnnFractionalEnsembledStrongest(object):
    ...
    def __init__(self, frecords, distance, find_closest_points, k_list, 
        ...
        PRINTER('[MlKnnFractionalEnsembledStrongest: init] labels: '+str(self.labels))
    ...
    def __calculate_label_counts(self):
        ...
        for r in self.frecords:
            ...
            for k in self.k_list:
                for code in self.classify_stupid(r, k):
                    d[code]+=1
    ...
    def __calculate_thresholds(self):
        ...
        for label in self.list_of_all_labels:
            ...
            for thresh in xrange(max_key+1):
                ...
                FP = sum([self.c[label][i] for i in xrange(thresh, max_key+1)])
                FN = sum([self.c_prim[label][i] for i in xrange(0, thresh)])
    ...
    def classify_stupid(self, sample, k):
        '''
    ...
    def classify_stupid_raw_no_exclude(self, sample, k):
        ...
        '''
        return self.find_closest_points(sample, self.frecords, [], k, self.distance.distance)

    def classify_stupid_no_exclude(self, sample, k):
        '''
        Classify element using KNN without use of precomputed probabilities and without excluding any points. 
        ...
        '''
        closest_samples = self.classify_stupid_raw_no_exclude(sample, k)
